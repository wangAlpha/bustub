...
TEST(BPlusTreeDeleteTests, DeleteBasic) {
  ...
  bpm->UnpinPage(HEADER_PAGE_ID, true);
  // EXPECT_EQ(true, bpm->CheckAllPinned());
  delete transaction;
  delete disk_manager;
  delete bpm;
  delete key_schema;
...
TEST(BPlusTreeDeleteTests, DeleteRandom) {
  ...
  GenericComparator<kSize> comparator(key_schema);

  DiskManager *disk_manager = new DiskManager("test.db");
  BufferPoolManager *bpm = new BufferPoolManager(50, disk_manager);
  // Create b+ tree
  BPlusTree<GenericKey<kSize>, RID, GenericComparator<kSize>> tree("foo_pk", bpm, comparator);

  GenericKey<kSize> index_key;
  RID rid;
  Transaction *transaction = new Transaction(0);
