...
TEST(BufferPoolManagerTest, SampleTest2) {
  ...
  ASSERT_NE(nullptr, root_page);
  strcpy(root_page->GetData(), "Hello\n");
  bpm->UnpinPage(root_page_id, true);

  Page *page = bpm->FetchPage(root_page_id);
  (void)page;
  EXPECT_EQ(root_page_id, root_page->GetPageId());
  printf("page id: %d, data: %s\n", root_page->GetPageId(), root_page->GetData());
  EXPECT_EQ(0, strcmp(root_page->GetData(), "Hello\n"));
